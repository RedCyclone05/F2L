...
# Leer el archivo CSV, procesar las columnas 6, 7 y 8 y luego escribir el resultado en un nuevo archivo
def process_csv(input_file, output_file):
    ...
    for i, row in enumerate(rows):
        if i == 0:  # Omitir el encabezado
            continue
        # Procesar columnas 6, 7 y 8 (índices 5, 6, 7)
        row[5] = replace_sequences(row[5])  # Columna 6
        row[6] = replace_sequences(row[6])  # Columna 7
        row[7] = replace_sequences(row[7])  # Columna 8
        row[9] = replace_sequences(row[9])  # Columna 10
